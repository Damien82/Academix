...
import DelegueTopbar from "../../../components/dashboard/DelegateTopbar"
import { useAuth } from "../../../context/AuthContext"
import { Lock, X, Edit, Check, AlertCircle, Mail, User, ShieldCheck, Camera, GraduationCap, FileText, Briefcase } from "lucide-react"
...
export default function DelegueProfile() {
  ...
  const [profileData, setProfileData] = useState({
    name: user?.fullName || "Délégué Utilisateur",
    email: user?.email || "delegue@institution.com",
  })

  // 🔹 Stats du délégué (Exemple statique)
  const stats = {
    rapportsSoumis: 12,
    coursAjoutes: 5,
  }
        ...
        <main className="flex-1 overflow-y-auto p-8 md:p-12 scrollbar-hide">
          <div className="max-w-4xl mx-auto space-y-10">
            ...
            <div className="flex items-end justify-between">
              <div>
                <h2 className="text-3xl font-extrabold text-slate-950 tracking-tight">Espace Délégué</h2>
                <p className="text-slate-500 font-medium mt-1">Gérez vos informations de délégué et supervisez vos activités.</p>
              </div>
            </div>
                ...
                <div className="grid md:grid-cols-3 gap-10">
                  ...
                  <div className="md:col-span-2 space-y-8">
                    <div>
                      ...
                      <div className="flex items-center gap-4 mt-2">
                        ...
                        <span className="px-2.5 py-1 bg-emerald-50 text-emerald-700 text-[10px] font-black uppercase rounded-lg flex items-center gap-1 border border-emerald-100">
                          <Briefcase size={12} /> {user?.role || "Délégué"}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100 hover:border-emerald-200 transition-colors group">
                        <GraduationCap className="text-slate-300 group-hover:text-emerald-500 mb-2 transition-colors" size={20} />
                        <p className="text-2xl font-black text-slate-950">{stats.coursAjoutes}</p>
                        <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Cours Ajoutés</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-900 rounded-[28px] p-8 text-white flex flex-col justify-between shadow-xl shadow-slate-200">
                    <div>
                      ...
                      <p className="text-slate-400 text-xs leading-relaxed">Protégez votre espace délégué en mettant régulièrement à jour votre mot de passe.</p>
                    </div>
                    <button
            ...
            <div className="relative bg-white rounded-[32px] w-full max-w-md p-10 shadow-2xl border border-slate-100 animate-in zoom-in duration-200">
              ...
              <div className="space-y-5">
                ...
                <div className="pt-6 flex gap-4">
                  ...
                  <button onClick={editProfile ? handleProfileUpdate : handlePasswordUpdate} className="flex-1 py-4 bg-emerald-600 text-white font-black rounded-xl text-xs shadow-lg shadow-emerald-100 hover:bg-emerald-700 transition-all active:scale-95">Confirmer</button>
                </div>
              </div>
            </div>
          </div>
